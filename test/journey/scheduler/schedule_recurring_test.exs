defmodule Journey.Scheduler.Scheduler.ScheduleRecurringTest do
  ...
  test "basic schedule_recurring flow" do
    ...
    assert {:ok, 1} = Journey.get_value(execution, :send_a_reminder, wait_any: 50_000)
    Process.sleep(1000)

    # Reminder 2 scheduled.
    {:ok, scheduled_time2} = Journey.get_value(execution, :schedule_a_reminder, wait_any: true)
    ...
    # Reminder 2 issued.
    assert {:ok, 2} = Journey.get_value(execution, :send_a_reminder, wait_any: 50_000)
    Process.sleep(1_000)

    # Reminded 3 was scheduled.
    {:ok, scheduled_time3} = Journey.get_value(execution, :schedule_a_reminder, wait_any: true)
