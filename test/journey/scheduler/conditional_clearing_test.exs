defmodule Journey.Scheduler.ConditionalClearingTest do
  ...
  describe "conditional clearing when upstream values change" do
    test "downstream node should be cleared when its condition is no longer met" do
      ...
      assert {:ok, %{value: 21}} = Journey.get_value(execution, :sum, wait_any: true)
      ...
      assert {:ok, %{value: 112}} = Journey.get_value(execution, :sum, wait_new: true)
      assert {:ok, %{value: "🚨"}} = Journey.get_value(execution, :large_value_alert, wait_any: true)

      # Decrease y so sum is below threshold again (12 + 1 = 13)
      execution = Journey.set(execution, :y, 1)

      # Wait for sum to be recomputed
      assert {:ok, %{value: 13}} = Journey.get_value(execution, :sum, wait_new: true)
    ...
    test "multiple conditional nodes should be managed correctly" do
      ...
      assert {:ok, %{value: "☂️"}} = Journey.get_value(execution, :bring_umbrella, wait_any: true)
      ...
      assert {:ok, %{value: "🕶️"}} = Journey.get_value(execution, :bring_sunglasses, wait_any: true)
