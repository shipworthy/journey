defmodule Journey.JourneySetValueMultiTest do
  ...
  describe "set_value with multiple values |" do
    test "basic multi-value setting" do
      ...
      assert {:ok, %{value: "Bros"}} = Journey.get_value(execution, :last_name)
      assert {:ok, %{value: "Mario Bros"}} = Journey.get_value(execution, :full_name, wait_any: true)
    end
    ...
    test "partial updates - only changed values increment revision once" do
      ...
      {:ok, %{value: "Mario Bros"}} = Journey.get_value(execution_v2, :full_name, wait_any: true)
      ...
      assert {:ok, %{value: "Luigi"}} = Journey.get_value(execution_v3, :first_name)
      assert {:ok, %{value: "Bros"}} = Journey.get_value(execution_v3, :last_name)
    end
    ...
    test "atomic updates with compute node dependency" do
      ...
      assert {:ok, %{value: "Mario"}} = Journey.get_value(execution, :first_name)
      assert {:ok, %{value: "Bros"}} = Journey.get_value(execution, :last_name)
      assert {:ok, %{value: 35}} = Journey.get_value(execution, :age)
      assert {:ok, %{value: "Mario Bros"}} = Journey.get_value(execution, :full_name, wait_any: true)
    end
    ...
    test "different value types" do
      ...
      assert {:ok, %{value: "hello"}} = Journey.get_value(execution, :text)
      assert {:ok, %{value: 42}} = Journey.get_value(execution, :number)
      assert {:ok, %{value: true}} = Journey.get_value(execution, :flag)
      assert {:ok, %{value: %{"key" => "value"}}} = Journey.get_value(execution, :data)
      assert {:ok, %{value: [1, 2, 3]}} = Journey.get_value(execution, :items)
    end
    ...
    test "using execution ID string" do
      ...
      assert {:ok, %{value: "Mario"}} = Journey.get_value(updated_execution, :first_name)
    end
    ...
    test "invalidates dependent computations correctly" do
      ...
      {:ok, %{value: "Mario Bros"}} = Journey.get_value(execution, :full_name, wait_any: true)

      # Change values - should invalidate and recompute
      execution = Journey.set(execution, %{first_name: "Luigi", last_name: "Mario"})
      {:ok, %{value: "Luigi Mario"}} = Journey.get_value(execution, :full_name, wait_new: true)
    end
    ...
    test "consistency: same result as individual set_value calls" do
      ...
      assert {:ok, %{value: val1_first}} = Journey.get_value(execution1, :first_name)
      assert {:ok, %{value: val2_first}} = Journey.get_value(execution2, :first_name)
      assert val1_first == val2_first

      assert {:ok, %{value: val1_last}} = Journey.get_value(execution1, :last_name)
      assert {:ok, %{value: val2_last}} = Journey.get_value(execution2, :last_name)
      assert val1_last == val2_last

      assert {:ok, %{value: val1_age}} = Journey.get_value(execution1, :age)
      assert {:ok, %{value: val2_age}} = Journey.get_value(execution2, :age)
      assert val1_age == val2_age

      # Computed values should be the same
      {:ok, %{value: full_name1}} = Journey.get_value(execution1, :full_name, wait_any: true)
      {:ok, %{value: full_name2}} = Journey.get_value(execution2, :full_name, wait_any: true)
