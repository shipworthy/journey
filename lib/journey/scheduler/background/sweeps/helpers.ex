defmodule Journey.Scheduler.Background.Sweeps.Helpers do
  ...
  def computations_for_graphs(nil, graph_names_and_versions) do
    ...
    conditions = filter_by_graphs_for_computations(graph_names_and_versions)
  ...
  def computations_for_graphs(execution_id, graph_names_and_versions) do
    # Build dynamic where clause for multiple graph name/version pairs
    graph_conditions = filter_by_graphs_for_computations(graph_names_and_versions)
  ...
  @doc false
  def executions_for_graphs(nil, graph_names_and_versions) do
    conditions = filter_by_graphs_for_executions(graph_names_and_versions)
    from(e in Execution, where: ^conditions)
  end

  @doc false
  def executions_for_graphs(execution_id, graph_names_and_versions) do
    graph_conditions = filter_by_graphs_for_executions(graph_names_and_versions)
    # Combine execution_id filter with graph filters using dynamic
    conditions = dynamic([e], e.id == ^execution_id and ^graph_conditions)
    from(e in Execution, where: ^conditions)
  end
