defmodule Journey.Repo.Migrations.AddExecutionsAndNodes do
  use Ecto.Migration

  def change do
    create table(:executions, primary_key: false) do
      add(:id, :string, primary_key: true)
      add(:graph_name, :string)
      add(:graph_version, :string)
      add(:revision, :integer, default: 0)
      timestamps(type: :bigint)
    end

    create(index(:executions, [:graph_name]))

    create table(:values, primary_key: false) do
      add(:id, :string, primary_key: true)
      add(:execution_id, :string)
      add(:node_name, :string)
      add(:node_type, :string)
      add(:node_value, :jsonb)
      add(:set_time, :integer, default: nil)
      add(:ex_revision, :integer, default: 0)
      timestamps(type: :bigint)
    end

    create(index(:values, [:execution_id, :node_name]))

    create table(:computations, primary_key: false) do
      add(:id, :string, primary_key: true)
      add(:execution_id, :string)
      add(:node_name, :string)
      add(:computation_type, :string)
      add(:state, :string, default: "not_set")
      add(:ex_revision_at_start, :integer, default: 0)
      add(:ex_revision_at_completion, :integer, default: 0)
      add(:scheduled_time, :integer)
      add(:start_time, :integer)
      add(:completion_time, :integer)
      add(:deadline, :integer)
      add(:error_details, :string)
      timestamps(type: :bigint)
    end

    create(index(:computations, [:execution_id, :node_name]))

    #    create table(:computations, primary_key: false) do
    #      add(:id, :string, primary_key: true)
    #      add(:execution_id, :string)
    #      add(:name, :string)
    #      add(:scheduled_time, :bigint)
    #      add(:start_time, :bigint)
    #      add(:end_time, :bigint)
    #      add(:deadline, :bigint)
    #      add(:result_code, :string)
    #      add(:result_value, :map)
    #      add(:error_details, :string)
    #      add(:ex_revision, :integer, default: 0)
    #      timestamps(type: :bigint)
    #    end
    #
    #    create(index(:computations, [:execution_id, :name]))
    #    create(index(:computations, [:start_time]))
    #    create(index(:computations, [:end_time]))
    #    create(index(:computations, [:deadline]))
    #    create(index(:computations, [:scheduled_time]))
    #
  end
end
