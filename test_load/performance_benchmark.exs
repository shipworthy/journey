defmodule LoadTest.PerformanceBenchmark do
  ...
  defp get_basic_db_stats() do
    ...
    executions_count = from(e in Journey.Schema.Execution, select: count(e.id)) |> Journey.Repo.one()
    values_count = from(v in Journey.Schema.Execution.Value, select: count(v.id)) |> Journey.Repo.one()
    computations_count = from(c in Journey.Schema.Execution.Computation, select: count(c.id)) |> Journey.Repo.one()
    ...
    query_tasks =
      ...
      |> Enum.map(fn _ ->
        Task.async(fn ->
          ...
          from(v in Journey.Schema.Execution.Value,
  ...
  defp run_scenario(:archive_unarchive_workload, _concurrency, iterations) do
    ...
    active_counts =
      1..5
      |> Enum.map(fn _ ->
        from(e in Journey.Schema.Execution,
  ...
  defp run_scenario(:scheduled_computation_load, _concurrency, iterations) do
    ...
    scheduled_count =
      from(c in Journey.Schema.Execution.Computation,
